...
const Backoffice = () => {
  ...
  useEffect(() => {
    const fetchAdmin = async () => {
      try {
        const res = await fetch("http://localhost:3002/admins/me", {
  ...
  const handleModalSubmit = async (formData) => {
    ...
    try {
      ...
      switch (modalType) {
        case "festival":
          url = modalData?.id ? `http://localhost:3002/festivals/${modalData.id}` : "http://localhost:3002/festivals";
          break;
        case "artist":
          url = modalData?.id ? `http://localhost:3002/artists/${modalData.id}` : "http://localhost:3002/artists";
          break;
        case "user":
          url = modalData?.id ? `http://localhost:3002/public-users/${modalData.id}` : "http://localhost:3002/public-users";
          break;
        case "admin":
          url = modalData?.id ? `http://localhost:3002/admins/${modalData.id}` : "http://localhost:3002/admins";
          break;
        case "lineup":
          url = modalData?.id ? `http://localhost:3002/lineups/${modalData.id}` : "http://localhost:3002/lineups";
          break;
        default:
          return;
      }
      ...
      let options = {
        method,
        headers: { Authorization: `Bearer ${token}` },
        body: hasFile ? new FormData() : JSON.stringify(formData),
      };
      ...
      case "user":
        return [
          ...
          { id: "country", label: "Country", value: data.country || "", onChange: (e) => setModalData({ ...data, country: e.target.value }) },
          {
            ...
            type: "file",
            value: "",
            onChange: (e) => setModalData({ ...data, profileImg: e.target.files[0] }),
          },
        ];
      case "admin":
        return [
          ...
          },
          {
            ...
            onChange: (e) => setModalData({ ...data, department: e.target.value }),
          },
        ];
      default:
        return [];
  ...
  return (
    <Container className="mb-5" style={{ minHeight: "80vh" }}>
      <Row className="align-items-center mb-3">
        <Col xs={12} md={8}>
          <FestiMateSearchbar value={search} onChange={setSearch} onSearch={handleSearch} />
        </Col>
