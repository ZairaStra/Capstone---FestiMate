  ...
  return (
    <Container className="mb-5" style={{ minHeight: "80vh" }}>
      ...
      <Row xs={1} sm={2} lg={3} xxl={4} className="g-4">
        {festivals.map((f) => (
          <Col key={f.id}>
            <FestiMateCard id={f.id} image={f.coverImg} title={f.name} subtitle={`${f.city}, ${f.country}`} linkPath="/festivals" isFestival={true} />
          </Col>
        ))}
      </Row>
...
const Festivals = () => {
  ...
  // Carica la wishlist all'inizio per avere gli ID
  useEffect(() => {
        ...
        {festivals.map((f) => (
          <Col key={f.id}>
            <FestiMateCard
              ...
              linkPath="/festivals"
              isFestival={true}
              initialWishlisted={wishlistIds.has(f.id)} // <-- qui la logica aggiunta
            />
          </Col>
        ))}
      </Row>
