  ...
  const handleSave = async () => {
    ...
    try {
      if (type === "file") {
        ...
        if (!oldPrice || !newPrice) throw new Error("Both old and new price are required");
        await onPatch({ oldPrice, newPrice });
    ...
    <>
      ...
      <FestiMateModal show={showModal} onClose={() => setShowModal(false)} title={`Update ${label || ""}`}>
        ...
        {type === "file" ? (
          ...
          <>
            <Form.Control type="number" value={oldPrice} onChange={(e) => setOldPrice(e.target.value)} placeholder="Old price" className="mb-3" />
            <Form.Control type="number" value={newPrice} onChange={(e) => setNewPrice(e.target.value)} placeholder="New price" />
            <div className="mt-3 text-end">
              <FestiMateButton onClick={handleSave} disabled={loading}>
                {loading ? <FestiMateSpinner /> : "Save"}
              </FestiMateButton>
